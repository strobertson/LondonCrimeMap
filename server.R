...
function(input, output, session){
  ...
  locationdata <- reactive({
    ...
    # Get a subset of the location data based on sidebar inputs
    locationData<-crime_location[crime_location$Month==input$month & crime_location$NAME!="City of London",]
      
    }
    else
      
    # Get a subset of the location data based on sidebar inputs
    locationData<-crime_location[crime_location$Month==input$month & crime_location$Crime.type==input$crime_group & crime_location$NAME!="City of London",]
  ...
  output$interactiveMap<-renderLeaflet({
    ...
    crime_popup <- paste0("<strong>Borough: </strong>", 
                          locData$NAME,
                          "<br><strong>",
                          "Month </strong>",
                          locData$Month,
  ...
  observe({
    ...
    if (input$crime_group == "All crime") { 
      ...
      pal <- colorNumeric("YlOrRd", theData$Total, n = 10) 
  ...
  output$fcdiag <- renderPlot({
    ...
    checkresiduals(fc, plot = TRUE) + theme_bw()
  ...
  output$predtab <- renderDataTable({
    ...
    datatable(round(fc, 2), extensions = 'Buttons', options = list(
      dom = 'Bfrtip',
